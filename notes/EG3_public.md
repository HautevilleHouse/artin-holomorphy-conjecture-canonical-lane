...
In-paper anchor: `paper/ARTIN_HOLOMORPHY_CONJECTURE_PREPRINT.md` (`AH_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving holomorphy of nontrivial Artin L-functions in the declared admissible class through an admissible Artin-regularity closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`AH_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_AH`.
...
If near-failure extraction and no-Zeno spacing hold, then `AH_G3` prevents hidden drift before rigidity is applied.
