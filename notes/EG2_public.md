...
In-paper anchor: `paper/ARTIN_HOLOMORPHY_CONJECTURE_PREPRINT.md` (`AH_G2`).

## Goal
Expand the compressed capture/restart language into the local-to-global transport gate for `proving holomorphy of nontrivial Artin L-functions in the declared admissible class through an admissible Artin-regularity closure architecture`.
...
`AH_G2` closes when `sigma_local` survives admissible losses and restart corrections: local-factor defect stays above capture floor across admissible ramification losses.
This is the transport contribution to `M_AH`.
...
If transport accounting and restart preservation hold, then `AH_G2` carries local control forward without breaking admissibility.
