...
In-paper anchor: `paper/ARTIN_HOLOMORPHY_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving holomorphy of nontrivial Artin L-functions in the declared admissible class through an admissible Artin-regularity closure architecture`.
...
- endpoint lock: `regularity_transfer`.
...
2. the endpoint-transfer lock `regularity_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_AH` remains positive.
...
Payload: verify `regularity_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_AH` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `regularity_transfer`
