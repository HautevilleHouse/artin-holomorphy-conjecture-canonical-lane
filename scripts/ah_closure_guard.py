#!/usr/bin/env python3
"""Canonical-lane closure guard for the artin holomorphy conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['AH_G1', 'AH_G2', 'AH_G3', 'AH_G4', 'AH_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['AH_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['AH_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['AH_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['AH_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['AH_G1'], 'G2': gates['AH_G2'], 'G3': gates['AH_G3'], 'G4': gates['AH_G4'], 'G5': gates['AH_G5'], 'G6': 'NA', 'GCoh': gates['AH_G6'], 'GM': gates['AH_GM']}
    normalized_blockers = {'G1': blockers['AH_G1'], 'G2': blockers['AH_G2'], 'G3': blockers['AH_G3'], 'G4': blockers['AH_G4'], 'G5': blockers['AH_G5'], 'G6': [], 'GCoh': blockers['AH_G6'], 'GM': blockers['AH_GM']}
    report = {'meta': {'computed_at_utc': dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(), 'framework': 'artin_holomorphy_conjecture'}, 'schema': {'normalized_gate_keys': ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'GCoh', 'GM'], 'g6_policy': 'NA means this framework has no standalone pre-coherence G6 gate'}, 'lane': {'canonical_theorem_lane': 'manifold_constrained', 'active_lane': 'manifold_constrained'}, 'inputs': {**primary_values, 'eps_coh': eps_coh, 'sigma_star_can': sigma_star}, 'derived': {'strict_margin': margin, 'strict_coh_zero': bool(strict_coh_zero)}, 'gates': gates, 'blockers': blockers, 'normalized': {'gates': normalized_gates, 'blockers': normalized_blockers}}
